...
def sample_v_given_u(adj: SparseTensor, u_indices: Tensor, prev_v: Tensor, 
                    num_neighbors: int, replace=False) -> Tuple[SparseTensor, Tensor]:
    
    # to homogenous adjacency
    nu, nv = adj.sparse_sizes()
    adj_h = SparseTensor(row = adj.storage.row(), 
                         col = adj.storage.col() + nu, 
                         value=adj.storage.value(),
                         sparse_sizes=(nu+nv, nu+nv))

    res_adj_h, res_id = adj_h.sample_adj(torch.cat([u_indices, prev_v + nu]), 
                                    num_neighbors=num_neighbors, replace=replace)
...
def sample_u_given_v(adj: SparseTensor, v_indices: Tensor, prev_u: Tensor, 
                    num_neighbors: int, replace=False) -> Tuple[SparseTensor, Tensor]:
    
    # to homogenous adjacency
    res_adj_t, u_indices = sample_v_given_u(adj.t(), v_indices, prev_u,
                                    num_neighbors=num_neighbors, replace=replace)
...
class BipartiteNeighborSampler(torch.utils.data.DataLoader):
    ...
    def sample_v_given_u(self, u_indices: Tensor, prev_v: Tensor, num_neighbors: int) -> Tuple[SparseTensor, Tensor]:
        
        res_adj_h, res_id = self.adj_homogen.sample_adj(torch.cat([u_indices, prev_v + self.nu]), 
                                        num_neighbors=num_neighbors, replace=False)
    ...
    def sample_u_given_v(self, v_indices: Tensor, prev_u: Tensor, num_neighbors: int) -> Tuple[SparseTensor, Tensor]:
        
        # start = time.time()
        res_adj_h, res_id = self.adj_t_homogen.sample_adj(torch.cat([v_indices, prev_u + self.nv]), 
                                        num_neighbors=num_neighbors, replace=False)
    ...
    def adjacency_from_samples(self, adj: SparseTensor, 
                               u_id: Tensor, v_id: Tensor, 
                               flow: str) -> DirectedAdj:
    ...
    def combine_adjacency(self, v2u_adj: SparseTensor, 
                                u2v_adj: SparseTensor, 
                                e_adj: SparseTensor
                                ) -> SparseTensor:
        ...
        row = torch.cat([e_adj.storage.row(), v2u_adj.storage.row(), u2v_adj.storage.row()], dim=-1)
        col = torch.cat([e_adj.storage.col(), v2u_adj.storage.col(), u2v_adj.storage.col()], dim=-1)
        value = torch.cat([e_adj.storage.value(), v2u_adj.storage.value(), u2v_adj.storage.value()], dim=0)
        fl = torch.cat([torch.ones(e_adj.nnz()), 2*torch.ones(v2u_adj.nnz()), 4*torch.ones(u2v_adj.nnz())])

        storage = SparseStorage(row=row, col=col, value=value,
                            sparse_sizes=(nu, nv), is_sorted=False)
        storage = storage.coalesce(reduce='mean') 

        fl_storage = SparseStorage(row=row, col=col, value=fl,
                            sparse_sizes=(nu, nv), is_sorted=False)
    ...
    def sample(self, batch):
        ...
        if self.base == 'u':
            ...
            u_indices = batch
            res_adj, res_id  = self.sample_v_given_u(u_indices, empty_list, num_neighbors=self.num_neighbors_u[0])
            ...
            v_indices = batch
            res_adj, res_id  = self.sample_u_given_v(v_indices, empty_list, num_neighbors=self.num_neighbors_v[0])
            rand_id = torch.randperm(len(res_id))[:self.n_other_node]
        ...
        for i in range(self.n_layers):
            ...
            # v -> u
            u_adj, next_v_indices  = self.sample_v_given_u(u_indices, prev_v=v_indices, num_neighbors=self.num_neighbors_u[i])
            dir_adj_v2u = self.adjacency_from_samples(u_adj, u_indices, next_v_indices, 'v->u')

            # u -> v
            v_adj, next_u_indices  = self.sample_u_given_v(v_indices, prev_u=u_indices, num_neighbors=self.num_neighbors_v[i])
            dir_adj_u2v = self.adjacency_from_samples(v_adj, next_u_indices, v_indices, 'u->v')
            
            # u -> e <- v
            dir_adj_e = self.adjacency_from_samples(e_adj, u_indices, v_indices, 'u->e<-v')

            # add them to the list
            adjacencies.append(BEANAdjacency(dir_adj_v2u, dir_adj_u2v, dir_adj_e)) 

            # for next iter
            e_adj, e_flag = self.combine_adjacency(v2u_adj=u_adj, u2v_adj=v_adj, e_adj=e_adj)
...
class EdgeLoader(torch.utils.data.DataLoader):
    ...
    def __init__(self, adj: SparseTensor, **kwargs):
        ...
        super().__init__(
            edge_idx.view(-1).tolist(), collate_fn=self.sample, **kwargs)        
