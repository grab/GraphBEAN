...
## sparse combine
def sparse_combine(a: SparseTensor, b: SparseTensor, 
        flag_mult: Optional[Tuple[int, int]] = (1,2)) -> Tuple[SparseTensor, SparseTensor]:
    
    res = spadd(a, b, op='mean')

    # flag where the source come from
    flag = spadd(a.fill_value(flag_mult[0]), 
                  b.fill_value(flag_mult[1]))
    
    return res, flag

## sparse combine
def sparse_combine3(a: SparseTensor, b: SparseTensor, c: SparseTensor
    ...
    # flag where the source come from
    flag = spadd(spadd(a.fill_value(flag_mult[0]), 
                  b.fill_value(flag_mult[1])), 
                  c.fill_value(flag_mult[2]))
    
    return res, flag

def sparse_combine3a(a: SparseTensor, b: SparseTensor, c: SparseTensor
                   ) -> Tuple[SparseTensor, SparseTensor, SparseTensor]:
...
def xe_split3(xe: Tensor, flag: Tensor,
