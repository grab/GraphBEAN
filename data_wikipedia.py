...
def prepare_data():
    df = pd.read_csv(f'data/wikipedia.csv')
    ...
    df_edge.to_csv(f'data/wikipedia-edge.csv')
    ...
    df_user.to_csv(f'data/wikipedia-user.csv')
    ...
    df_item.to_csv(f'data/wikipedia-item.csv')


def create_graph():

    df_user = pd.read_csv('data/wikipedia-user.csv')
    df_item = pd.read_csv('data/wikipedia-item.csv')
    df_edge = pd.read_csv('data/wikipedia-edge.csv')
...
def synth_random():
    ...
    prepare_data()
    graph = create_graph()
    store_graph('wikipedia-graph', graph)
    # graph = torch.load(f'storage/wikipedia-graph.pt')

    graph_anomaly_list = []
    for i in range(args['n_graph']):
        print(f"GRAPH ANOMALY {i} >>>>>>>>>>>>>>")
        print(graph)
        graph_multi_dense = inject_random_block_anomaly(graph,num_group=20, num_nodes_range=(1, 20), num_nodes_range2=(1,6))
