...
parser = argparse.ArgumentParser(description="AdONE")
...
torch.save(
    model_stored,
    os.path.join(result_dir, f"adone-{args['name']}-{args['id']}-model.th"),
)
torch.save(
    output_stored,
    os.path.join(result_dir, f"adone-{args['name']}-{args['id']}-output.th"),
)
