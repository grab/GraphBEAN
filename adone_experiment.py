...
model = AdONE(hid_dim=args['hidden_channels'], dropout=args['dropout_prob'], 
            epoch=args['n_epoch'], lr=args['lr'], verbose=False, gpu=args['gpu'],
...
torch.save(model_stored, os.path.join(data_dir, f"adone-{args['name']}-{args['id']}-model.th"))
torch.save(output_stored, os.path.join(data_dir, f"adone-{args['name']}-{args['id']}-output.th"))
