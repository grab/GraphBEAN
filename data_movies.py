...
def prepare_data():
    ...
    df["helpfulness"] = (df["helpfulness_numerator"] / df["helpfulness_denominator"]).fillna(0)
    ...
    df_product = dfu.groupby("product_id", as_index=False).agg(
        user_count=('user_id', 'count'), helpful_num_mean=('helpfulness_numerator', 'mean'), helpful_num_sum=('helpfulness_numerator', 'sum'),
        helpful_mean=('helpfulness', 'mean'), helpful_sum=('helpfulness', 'sum'), score_mean=('score', 'mean'), score_sum=('score', 'sum'),
        summary_len_mean=('summary_char_len', 'mean'), summary_len_sum=('summary_char_len', 'sum'), text_len_mean=('text_char_len', 'mean'), text_len_sum=('text_char_len', 'sum')
        )
    
    df_user = dfu.groupby("user_id", as_index=False).agg(
        product_count=('product_id', 'count'), helpful_num_mean=('helpfulness_numerator', 'mean'), helpful_num_sum=('helpfulness_numerator', 'sum'),
        helpful_mean=('helpfulness', 'mean'), helpful_sum=('helpfulness', 'sum'), score_mean=('score', 'mean'), score_sum=('score', 'sum'),
        summary_len_mean=('summary_char_len', 'mean'), summary_len_sum=('summary_char_len', 'sum'), text_len_mean=('text_char_len', 'mean'), text_len_sum=('text_char_len', 'sum')
        )
...
def create_graph():
    ...
    df_review = pd.concat([df_review_id, pd.DataFrame(embeddings, columns=cols)], axis=1)
...
def synth_random():
    ...
    parser.add_argument("--name", type=str, default="movies_anomaly",
                        help="name")
    parser.add_argument("--n-graph", type=int, default=2,
                        help="n graph")
