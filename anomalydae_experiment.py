...
model = AnomalyDAE(embed_dim=args['hidden_channels'], out_dim=args['hidden_channels'],
...
torch.save(model_stored, os.path.join(data_dir, f"anomalydae-{args['name']}-{args['id']}-alpha-{args['alpha']}-model.th"))
torch.save(output_stored, os.path.join(data_dir, f"anomalydae-{args['name']}-{args['id']}-alpha-{args['alpha']}-output.th"))
