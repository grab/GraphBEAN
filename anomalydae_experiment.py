...
parser = argparse.ArgumentParser(description="AnomalyDAE")
...
parser.add_argument(
    "--num-neighbors", type=int, default=-1, help="number of neighbors for node"
)
...
xuh = torch.cat(
    [
        scatter(xe, adj.storage.row(), dim=0, reduce="max"),
        scatter(xe, adj.storage.row(), dim=0, reduce="mean"),
    ],
    dim=1,
)
xvh = torch.cat(
    [
        scatter(xe, adj.storage.col(), dim=0, reduce="max"),
        scatter(xe, adj.storage.col(), dim=0, reduce="mean"),
    ],
    dim=1,
)
...
anomaly_score = {"score_u": score_u, "score_v": score_v, "score_e": score_e}
...
output_stored = {"args": args, "anomaly_score": anomaly_score}
...
torch.save(
    ...
    os.path.join(
        result_dir,
        f"anomalydae-{args['name']}-{args['id']}-alpha-{args['alpha']}-model.th",
    ),
)
torch.save(
    output_stored,
    os.path.join(
        result_dir,
        f"anomalydae-{args['name']}-{args['id']}-alpha-{args['alpha']}-output.th",
    ),
)
