...
def sample_data():
    ...
    product_nums = np.random.choice(df_product.shape[0], 12000, replace=False, p=product_weight)
    product_ids = df_product['ProductId'][product_nums]

    df_review_chosen = df_review[df_review['ProductId'].isin(product_ids) & df_review['UserId'].isin(user_ids)].iloc[:,1:]
    df_user_chosen = df_user[df_user['UserId'].isin(df_review_chosen['UserId'].unique())].iloc[:,1:]
    df_product_chosen = df_product[df_product['ProductId'].isin(df_review_chosen['ProductId'].unique())].iloc[:,1:]
...
def synth_random():
    ...
    parser.add_argument("--name", type=str, default="finefoods-small_anomaly",
                        help="name")
