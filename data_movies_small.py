...
def sample_data():
    ...
    product_nums = np.random.choice(
        df_product.shape[0], 14000, replace=False, p=product_weight
    )
    product_ids = df_product["product_id"][product_nums]

    df_review_chosen = df_review[
        df_review["product_id"].isin(product_ids) & df_review["user_id"].isin(user_ids)
    ].iloc[:, 1:]
    df_user_chosen = df_user[
        df_user["user_id"].isin(df_review_chosen["user_id"].unique())
    ].iloc[:, 1:]
    df_product_chosen = df_product[
        df_product["product_id"].isin(df_review_chosen["product_id"].unique())
    ].iloc[:, 1:]
...
def synth_random():
    ...
    parser.add_argument("--name", type=str, default="movies-small_anomaly", help="name")
